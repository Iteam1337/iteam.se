...
describe('#team', function () {
  ...
  it('should call options.fn with the correct parameter', function () {
    front.loadFront.withArgs('./src/pages/medarbetare/rickard/index.hbs').returns({ email: 'rickard.laurin@iteam.se' });
    front.loadFront.withArgs('./src/pages/medarbetare/dennis/index.hbs').returns({ email: 'dennis.pettersson@iteam.se' });
    ...
    expect(options.fn).calledOnce.and.calledWith({
      data: [{
        ...
        url: '/medarbetare/rickard'
      }, {
        frontmatter: { email: 'dennis.pettersson@iteam.se' },
        img: 'http://www.gravatar.com',
        url: '/medarbetare/dennis'
      }]
    });
  });
});